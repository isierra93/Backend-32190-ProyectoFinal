...
export default class CarritosDAO extends MongoContainer {
  constructor() {
    super();
  }
  ...
  async crearCarrito(dueñoId) {
    try {
      await this.connect();
      let carrito = await model.carritos.create({
        productos: [],
        dueño: dueñoId,
      });
  ...
  async getById(carritoId) {
    try {
      await this.connect();
      let carrito = await model.carritos.findOne(
        { _id: carritoId },
        { __v: 0 }
      );
      await this.disconnect();
  ...
  async getCarritoIdByDueño(dueñoId) {
    try {
      await this.connect();
      let carritoId = await model.carritos.findOne(
        { dueño: dueñoId },
        { _id: 1 }
      );
      await this.disconnect();
  ...
  async añadirProducto(carritoId, prodId) {
    try {
      ...
      let res = await model.carritos.updateOne(
        { _id: carritoId },
